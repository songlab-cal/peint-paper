...
#   2. fold + 3Di + pLDDT                 protevo-env   GPU   (sims are cache hits here)
...
: "${PEINT_PAPER_PY_PROTEVO:?set it to the protevo-env interpreter}"
...
# omegafold is a console script in protevo-env/bin, which is not on PATH when the interpreter
# is invoked by absolute path. Put that bin first for step 2.
PROTEVO_BIN="$(dirname "$PEINT_PAPER_PY_PROTEVO")"
...
if step 2; then
  run "2/4 structures + 3Di  (protevo-env; simulations are cache hits)"
  PATH="$PROTEVO_BIN:$PATH" "$PEINT_PAPER_PY_PROTEVO" -m benchmarks.generate_all_results \
