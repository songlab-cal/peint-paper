...
local_data/
  ...
  vep/           per-run ProteinGym Spearman tables (ships in-repo, ~330 KB)
...
On a machine that already holds the source trees, `scripts/link_local_data.sh` builds that
layout as symlinks instead of copies. It is dry-run by default, only ever creates symlinks,
and refuses to touch anything that is not already one:

```bash
scripts/link_local_data.sh            # show what it would do
scripts/link_local_data.sh --apply
scripts/link_local_data.sh --check    # verify every link still resolves
```

Only `derived/` is written to; `r1`, `r2`, `sim`, `a3m`, `pdb` and `annotations` are inputs.
